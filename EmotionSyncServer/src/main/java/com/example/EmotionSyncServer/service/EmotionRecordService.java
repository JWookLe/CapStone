...
public class EmotionRecordService {
     ...
     * 사용자의 최신 감정 기록 조회
     * @param userId 사용자 ID
     * @return 최신 감정 기록 (없으면 null)
     */
    public EmotionRecord getLatestEmotionRecord(String userId) {
        Optional<User> userOptional = userRepository.findById(userId);
        
        if (userOptional.isPresent()) {
            return emotionRecordRepository.findTopByUserOrderByRecordedAtDesc(userOptional.get());
        }
